...
7 Player2 Deck & Discard remain the same  */ 
    ...
    printf("----------------- Unit Test 3 - Testing Refactored councilRoom Function ----------------\n\n");
    ...
    printf("Printing Cards BEFORE councilRoom function is called: \n");
    ...
    playCouncilRoomCard(thisPlayer, &testG, handpos);

    /*-------------------------------------------------------------------------------------*/ 

    /* print cards after adventurerCard is played  */
   /* Print Player 1 Hand & Details After Smithy Function */ 
    printf("Printing Cards AFTER councilRoom function is called: \n");
    for (m=0; m<testG.handCount[thisPlayer]; m++) {
            printf("(%d)", testG.hand[thisPlayer][m]);}
    printf("\n");
    printf("\n");
    ...
    printf("TEST 1: Checking for Drawing of 4 Cards \n");

    if (postHandCount - preHandCount ==3) // discard one to playedCard so 3 - 1 = 2 
    {
        printf("councilRoomCard(): PASS - Hand was 4 Cards bigger after the function than before as expected\n");
        score++;}
    else
        printf("councilRoomCard(): FAIL - Hand was not 4 Cards bigger after the function than before as expected\n");
    ...
    if (preDeckCount+preHandCount+prePlayedCardCount+preDiscardCount == postPlayedCardCount+postDeckCount+postHandCount+postDiscardCount) {
        printf("councilRoomCard(): PASS - All Cards Accounted for, no cards lost\n"); 
        score++; }
    else
        printf("councilRoomCard(): FAIL - Not all Cards Accounted for, cards lost\n"); 
    ...
    if (playerTurnPre==playerTurnPost) {
        printf("councilRoomCard(): PASS - Function did not change the player Turn as expected\n"); 
        score++; }
    else 
        { printf("councilRoomCard(): FAIL - Function changed the player turn in error\n");}

    printf("\n");

    /* ----------- TEST 4: Affirming Buys are not Changed -------------- */
    printf("TEST 4: Affirming Buys are Increased by 1 \n");

    if (preNumBuys+1==postNumBuys) {
        printf("councilRoomCard(): PASS - Function added one to current player buys count as expected\n"); 
        score++;}
    else 
        { printf("councilRoomCard(): FAIL - Function changed the player buys count in error\n"); }
    ...
    if (preProvince==postProvince && preDuchy==postDuchy && preEstate==postEstate) {
        printf("councilRoomCard(): PASS - Function did not change the player supply cards count as expected\n"); 
        score++; }
    else 
        { printf("councilRoomCard(): FAIL - Function changed the player supply cards count in error\n");}
    ...
    if (prePhase==postPhase) {
        printf("councilRoomCard(): PASS - Function did not change the game phase as expected\n"); 
        score++; }
    else 
        { printf("councilRoomCard(): FAIL - Function changed the game phase in error\n");}

    printf("\n");


    /* ----------- TEST 7: Affirming Other Player Draws One Card -------------- */
    printf("TEST 8: Affirming Other Player Draws One Card in Function \n");

    if (otherDeckPre-1==otherDeckPost) {
        printf("councilRoomCard(): PASS - Function made the other player draw a card as expected\n"); 
        score++; }
    else 
        { printf("councilRoomCard(): FAIL - Function did not have the other player draw a card\n"); }
